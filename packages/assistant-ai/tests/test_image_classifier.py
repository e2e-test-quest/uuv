...
def analyze_analysis_details_similarity(
    example: dspy.Example, prediction: dspy.Prediction
) -> float:
    """Using LLM As Judge for similarity scoring of analysis_details."""
    llm_judge = dspy.Predict(
        "expected_analysis_details, predicted_analysis_details-> similarity_score: float" # noqa E501.
    )

    judge_lm = dspy.LM(
        model=os.environ.get("JUDGING_LLM_MODEL"),
        api_key=os.environ.get("JUDGING_LLM_API_KEY"),
        api_base=os.environ.get("JUDGING_LLM_API_URL"),
    )
    llm_judge.set_lm(judge_lm)

    result = llm_judge(
        expected_analysis_details=example.expected_analysis_details,
        predicted_analysis_details=prediction.analysis_details,
    )
...
def comprehensive_metric(
    ...
    """
    Comprehensive metric function using LLM-as-judge for analysis_details similarity.

    Weighted scoring:
    - is_decorative: 0.5 weight (most important - boolean match)
    - confidence: 0.3 weight (proximity score)
    - analysis_details: 0.2 weight (LLM judge similarity)
    """
    is_decorative_score = (
        1.0 if prediction.is_decorative == example.expected_is_decorative else 0.0
    )

    confidence_score = max(
        0.0, 1.0 - abs(prediction.confidence - example.expected_confidence) * 20
    )

    similarity = analyze_analysis_details_similarity(example, prediction)

    total_score = is_decorative_score * 0.5 + confidence_score * 0.3 + similarity * 0.2

    if trace is None:
        return total_score
    else:
        return (
            is_decorative_score >= 1.0
...
def evaluate_classifier(
    ...
    result = evaluate(
        agent,
        metric=lambda ex, pred, trace=None: comprehensive_metric(
            ex, pred, trace
        ),
    )
