...
function drawNavigationOrder(
  layerContainer: Element,
  targetElement: FocusableElement,
  order: number,
) {
  const x =
    window.scrollX +
  ...
  const orderIndicatorContainer = document.createElementNS(
    SVG_NAMESPACE,
    "circle",
  );
...
function drawLinkBetweenElement(
  layerContainer: Element,
  startPoint: Point,
  endPoint: Point,
) {
...
function initSvgElement(
  ...
  const layerShadowRootElement = layerShadowRoot.querySelector(
    `#${layer.toString()}`,
  );
...
export function buildLayer(
  ...
  const layerContainer = layerShadowRoot.querySelector(
    `#${layer.toString()}-svg`,
  );
  if (layerContainer) {
...
export function addLayerToShadowDom(
  dom: ShadowRoot,
  layer: AdditionalLayerEnum,
) {
...
export function removeLayerToShadowDom(
  dom: ShadowRoot,
  layer: AdditionalLayerEnum,
) {
