...
import { ScenarioResult, ScenarioResultSchema, Step } from "./architect.model";
...
async function analyzeScenario(targetUrl: string, scenario: string, tools: any, llmModel: string, llmApi?: string): Promise<string> {
    ...
    const { output } = await generateText({
      ...
      prompt: `
        ...
        Trace:
        ${JSON.stringify(editedStepOutputs, null, 2)}

        Return JSON matching the schema.
      `,
      output: Output.object({ schema: ScenarioResultSchema }),
    });

    logger.debug("output");
    logger.debug(output);

    return formatScenario(output);
}
